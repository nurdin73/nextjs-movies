...
function ListPersonByMovie({getListCredits, getDetailMovie}) {
    ...
    return (
        <Fragment>
            <Head>
                <title>{getDetailMovie.name || getDetailMovie.original_name} - Cast & crew | LUX online movie rating</title>
                <meta name="description" content={getDetailMovie.overview} />
                <meta property="og:title" id="titleOg" content={`${getDetailMovie.name || getDetailMovie.original_name} - Cast & crew | LUX online movie rating`} />
...
export async function getServerSideProps(context) {
    ...
    const credits = await fetch(`https://api.themoviedb.org/3/tv/${tv_id}/credits?api_key=${process.env.API_KEY}`)
    const getListCredits = await credits.json()

    const detailMovie = await fetch(`https://api.themoviedb.org/3/tv/${tv_id}?api_key=${process.env.API_KEY}`)
