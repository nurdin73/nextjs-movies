...
function Btn({title, icon, media_id, media_type, type_btn}) {
    ...
    }, [setAccountStates, media_type, media_id])

    const handleSetMark = (media, id, type, status) => () => {
    ...
    if(type_btn === "favorite") {
        ...
        return (
            <Fragment>
                <li ref={setTriggerRef} onClick={handleSetMark(media_type, media_id, type_btn, accountStates?.rated)} className="bg-gray-900 p-3 group rounded-full cursor-pointer shadow transform transition duration-300">
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${accountStates?.rated ? "text-yellow-500" : "text-gray-500"} group-hover:text-yellow-500`} viewBox="0 0 20 20" fill="currentColor">
                        {icon}
                    </svg>
                    {visible && (
                        <div
